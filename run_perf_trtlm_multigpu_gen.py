...
def parse_arguments(args=None):
    ...
    parser.add_argument('--profiling_tag', type=str, default='')
    parser.add_argument('--static_input_batch', type=int, default=1)
...
def parse_input(tokenizer,
    ...
    if input_file is None:
        for curr_text in input_text:
            ...
            batch_input_ids *= args.static_input_batch
...
def main(args):
    ...
    if args.streaming:
        ...
        print('---DDD--- current runtime_rank: ', runtime_rank)
        if runtime_rank == 0:
    ...
    if args.run_profiling:
        ...
        for _ in range(ite):
            ...
            tensorrt_llm.profiler.start("gpuinf")
            with torch.no_grad():
                outputs = runner.generate(
                    batch_input_ids,
                    max_new_tokens=args.max_output_len,
                    ...
                    return_dict=True)
                # torch.cuda.synchronize()
            tensorrt_llm.profiler.stop("gpuinf")
            gpuinf_secs.append(tensorrt_llm.profiler.elapsed_time_in_sec('gpuinf'))
        ...
        if runtime_rank == 0:
            ...
            output_lengths = [x.size(1) for x in output_ids]
            new_output_lengths = [output_lengths[i] - input_lengths[i] for i in range(len(output_lengths))]

            tot_elaps_sec = tensorrt_llm.profiler.elapsed_time_in_sec('tmp')

            logstr = f'''tid: {args.profiling_tag}; batch_size: {len(input_lengths)}; avg latency: {tot_elaps_sec / ite:.4f}; \
    avg inf latency: {tensorrt_llm.profiler.elapsed_time_in_sec('gpuinf')/ite:.4f}; \
    throughput: {sum(new_output_lengths)/tot_elaps_sec:.4f}; \
    Input len: {input_lengths[0]}; New output len: {new_output_lengths[0]}; QPS: {len(input_lengths)/tensorrt_llm.profiler.elapsed_time_in_sec('gpuinf')/ite:.4f}. \n''' # {gpuinf_secs}'''

            # os.system(f'echo {logstr} >> perf-profilings.log')

            with open(f'perf-profilings.log','a') as f:
                f.write(logstr)
